...
public static class SnapshotContractV1
{
    ...
    public static string Serialize(SnapshotEnvelopeV1 envelope)
    {
        Validate(envelope);
    ...
    public static SnapshotEnvelopeV1 Deserialize(ReadOnlySpan<byte> json)
    {
        ...
        try
        {
            envelope = JsonSerializer.Deserialize<SnapshotEnvelopeV1>(json, SerializerOptions)
                ?? throw new SnapshotContractValidationException(["Payload must be a JSON object."]);
        }
        catch (JsonException exception)
        {
            throw new SnapshotContractValidationException(
                ["Payload does not match snapshot schema version 1."], exception);
        }
    ...
    private static void ValidateObject(object? value, string path, List<string> errors, HashSet<object> visited)
    {
        ...
        {
            errors.Add($"{path}: Number must be finite.");
            return;
        }
    ...
    {
        if (envelope.SchemaVersion != SnapshotSchemaVersions.V1)
            errors.Add("$.schemaVersion: Only schema version 1 is supported.");
        if (envelope.CollectedAt == default || envelope.CollectedAt.Offset != TimeSpan.Zero)
            errors.Add("$.collectedAt: Timestamp must be a non-default UTC value.");
        if (envelope.CollectorEpoch == Guid.Empty)
            errors.Add("$.collectorEpoch: Collector epoch must not be empty.");
        ...
        foreach (var player in envelope.Snapshot.Players?.Data ?? [])
        {
            ...
            else ValidatePlayerLocation(player.Location, errors);
        }
        foreach (var guild in envelope.Snapshot.World?.Data?.Guilds ?? [])
        {
            if (guild is null) { errors.Add("$.snapshot.world.data.guilds: Entries must not be null."); continue; }
            if (guild.Bases.Any(item => item is null)) errors.Add("$.snapshot.world.data.guilds[].bases: Entries must not be null.");
        }
        ValidateServerRules(envelope.Snapshot.Server?.Data?.Rules, errors);
    }
    ...
    private static void ValidateSection<T>(SnapshotSection<T>? section, string path, List<string> errors)
    {
        ...
        if (status.LastAttemptedAt is { Offset: var attemptedOffset } && attemptedOffset != TimeSpan.Zero ||
            status.LastSuccessfulAt is { Offset: var successfulOffset } && successfulOffset != TimeSpan.Zero)
            errors.Add($"{path}.status: Timestamps must use UTC.");
        if (status.State == SnapshotSourceState.Pending &&
            (status.IsStale || status.LastAttemptedAt is not null || status.LastSuccessfulAt is not null || section.Data is not null))
            errors.Add($"{path}: Pending sections cannot contain attempts, stale state, or data.");
        if (status.State == SnapshotSourceState.Healthy &&
            (status.IsStale || status.LastAttemptedAt is null || status.LastSuccessfulAt is null || section.Data is null))
            errors.Add($"{path}: Healthy sections require current data and a successful timestamp.");
        if (status.IsStale && (status.LastSuccessfulAt is null || section.Data is null))
            errors.Add($"{path}: Stale sections require retained successful data.");
        if (status.State is SnapshotSourceState.Unauthorized or SnapshotSourceState.Unavailable)
        {
            ...
            if (section.Data is not null && !status.IsStale) errors.Add($"{path}: Retained data after a failure must be marked stale.");
        }
    }

    private static void ValidatePlayerLocation(PlayerLocation location, List<string> errors)
    {
        var valid = location?.Kind switch
        {
            PlayerLocationKind.Overworld => location.Layer is not null && IsFinite(location.X) && IsFinite(location.Y) && location.Stage is null,
            PlayerLocationKind.Instance => location.Layer is null && location.X is null && location.Y is null && !string.IsNullOrWhiteSpace(location.Stage),
    ...
    private static void ValidateServerRules(PublicServerRules? rules, List<string> errors)
    {
