...
| `PalmapIngest:SuppressIdleSnapshots` | `false` | After one accepted complete healthy-empty snapshot, suppress equivalent routine reports while local polling continues |
| `PalmapIngest:IdleSnapshotHeartbeatIntervalMs` | `21600000` | When idle suppression is enabled, send a new healthy-empty baseline at least every six hours |
...
Idle snapshot suppression is disabled by default. Enable it only when the connected hosted service models an accepted healthy-empty snapshot as quiet for at least the configured heartbeat window. A collector first sends a complete healthy-empty baseline and waits for `202 Accepted`; it then suppresses timestamp, sequence, world-refresh, and unchanged settings churn while continuing every local poll and health check. Players, source degradation or recovery, changed public server settings, process restart, or the heartbeat deadline resume delivery immediately. A rejected, timed-out, or failed baseline never enables silence. The six-hour default trades fewer hosted wakeups for up to a six-hour quiet-versus-dead detection window, so choose the interval deliberately.
